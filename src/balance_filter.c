...
void balance_filter_update(BalanceFilterData *data, const MotorData *motor, float *gyro_xyz, float *accel_xyz, float dt) {
    ...
    if (data->end_kp_pitch > 0 && data->dynamic_kp_erpm_end > data->dynamic_kp_erpm_start) {
        if (motor->abs_erpm < data->dynamic_kp_erpm_start) {
            ...
            float erpm_ratio = (motor->abs_erpm - data->dynamic_kp_erpm_start) / (data->dynamic_kp_erpm_end - data->dynamic_kp_erpm_start);
            data->kp_pitch = data->start_kp_pitch + (data->end_kp_pitch - data->start_kp_pitch) * erpm_ratio;
        } else {
